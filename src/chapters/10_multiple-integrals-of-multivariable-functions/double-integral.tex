...
\begin{problem}{利用对称性计算二重积分}{Double-Integral-Symmetry}
    利用函数的奇偶性计算下列积分：
    \begin{enumerate}
        \item $\iint_{D} (x^{2}+y^{2}) \d x \d y$，$D: -1 \leqslant x \leqslant 1, -1 \leqslant y \leqslant 1$；
...
\begin{solution}
    ...
    \end{equation}
    由 \cref{eq:parity-check} 可知，$g(x, y)$ 是关于 $x$ 的奇函数。根据二重积分的对称性质，奇函数在关于坐标轴对称的区域上的积分为零。故有
    \begin{equation}
...
\begin{myproof}
    ...
    由于 $\varphi(x)$ 与 $\psi(y)$ 分别在闭区间上可积，故它们在该区间上必有界。设存在常数 $M_1, M_2 > 0$ 使得 $|\varphi(x)| \leqslant M_1$ 且 $|\psi(y)| \leqslant M_2$，则有
    \begin{equation}
        |f(x, y)| = |\varphi(x)| \cdot |\psi(y)| \leqslant M_1 M_2,
    \end{equation}
...
\begin{myproof}
    ...
    \end{equation}
    利用变量的对等性，将 \cref{eq:Ident-1} 中的 $x$ 与 $y$ 互换，有
    \begin{equation}
        I = \int_0^a \d x \int_x^a f(y)f(x) \d y \label{eq:Ident-2}
    \end{equation}
    将原式与 \cref{eq:Ident-2} 相加，得
    \begin{equation}
        \begin{aligned}
