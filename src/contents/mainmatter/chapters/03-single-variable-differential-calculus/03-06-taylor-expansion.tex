...
\begin{problem}{泰勒公式条件的必要性分析}{Taylor-Formula-Condition-Necessity-Analysis}
    设 $n$ 为正整数，考虑函数
    \begin{equation}
        f(x) = \begin{dcases} x^{n+1}, & x \text{ 为有理数}, \\ 0, & x \text{ 为无理数}. \end{dcases}
    \end{equation}
...
\begin{myproof}
    ...
    由导数定义，考察差商：
    \begin{equation}
        \frac{f(x) - f(0)}{x - 0} = \frac{f(x)}{x} = \begin{dcases} x^n, & x \in \symbb{Q} \setminus \{0\}, \\ 0, & x \in \symbb{R} \setminus \symbb{Q}. \end{dcases}
    \end{equation}
    对任意 $x \neq 0$，恒有
    \begin{equation}
...
\begin{problem}{光滑非解析函数在原点的各阶导数}{Smooth-Non-Analytic-Function-Derivatives}
    考虑函数
    \begin{equation}
        f(x) = \begin{dcases} \e^{-\frac{1}{x^2}}, & x \neq 0, \\ 0, & x = 0, \end{dcases}
    \end{equation}
