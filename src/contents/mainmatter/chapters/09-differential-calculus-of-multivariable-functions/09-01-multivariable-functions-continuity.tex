...
\begin{problem}{拓扑正弦曲线}{Topologists-Sine-Curve}
    证明：集合 $E = \{(x,y) : y = \sin \frac{1}{x}, 0 < x \le \frac{2}{\uppi}\} \cup \{(0,y) : 0 \le y \le 1\}$ 是连通的但不是道路连通的．
...
\begin{myproof}
    令 $E_1 = \left\{ \left(x, \sin \frac{1}{x}\right) : 0 < x \le \frac{2}{\uppi} \right\}$ 且 $L = \{(0, y) : 0 \le y \le 1\}$，则 $E = E_1 \cup L$．
    ...
    定义 $t_0 = \sup \{ t \in [0, 1] : x(t) = 0 \}$．由 $x(t)$ 的连续性及 $x(1) > 0$ 知 $0 \le t_0 < 1$ 且 $x(t_0) = 0$．
...
\begin{solution}
    ...
    \begin{equation}
        \begin{dcases} (x+1)^2+y^2 \ge 1 \\ (x-1)^2+y^2 < 1 \end{dcases}
    \end{equation}
...
\begin{note}{关于“区域”定义的讨论}{Region-Definition}
    ...
    \begin{equation}
        D = \{ (x,y) \in \symbb{R}^2 \mid x^2 + y^2 \le 1 \text{ 或 } (x,y) = (t,0), t \in [1,2] \}.
    \end{equation}
    ...
    对其内部取闭包可得
    \begin{equation}
        \overline{D^{\circ}} = \{ (x,y) \in \symbb{R}^2 \mid x^2 + y^2 \le 1 \} \neq D.
    \end{equation}
...
\begin{problem}{复合函数解析式}{Composite-Function}
    设 $f(x, y) = \begin{dcases} 1, & y \ge x, \\ 0, & y < x, \end{dcases}$ 又 $\begin{dcases} x = \cos t, \\ y = \sin t, \end{dcases}$ 求 $F(t) = f(\cos t, \sin t)$．
\end{problem}

\begin{solution}
    根据定义 $F(t) = f(\cos t, \sin t)$，需比较 $\sin t$ 与 $\cos t$ 的大小关系：
    \begin{equation}
        \sin t \ge \cos t \iff \sqrt{2}\sin\left( t - \frac{\uppi}{4} \right) \ge 0 \iff 2k\uppi + \frac{\uppi}{4} \le t \le 2k\uppi + \frac{5\uppi}{4}, \quad k \in \symbb{Z}
    \end{equation}
    因此，函数 $F(t)$ 的表达式为：
    \begin{equation}
        \boxed{F(t) = \begin{dcases} 1, & 2k\uppi + \frac{\uppi}{4} \le t \le 2k\uppi + \frac{5\uppi}{4} \\ 0, & 2k\uppi - \frac{3\uppi}{4} < t < 2k\uppi + \frac{\uppi}{4} \end{dcases} \quad (k \in \symbb{Z})}
    \end{equation}
\end{solution}
...
\begin{problem}{极坐标下的极限}{Polar-Limit}
    若 $x = r \cos \theta, y = r \sin \theta$，问沿怎样的方向 $\theta$ ($0 \le \theta \le 2\uppi$)，下列极限存在？
    \begin{enumerate}
...
\begin{solution}
    ...
    由 $\cos 2\theta < 0$ 且 $0 \le \theta \le 2\uppi$ 可解得
    \begin{equation}
    ...
    \end{equation}
    若 $\cos 2\theta < 0$，则 $\e^{r^2 \cos 2\theta} \to 0$，而 $|\sin(r^2 \sin 2\theta)| \le 1$，由夹逼定理知 $L = 0$；\\
...
\begin{myproof}
    ...
    令 $\psi(x) =  \lim_{y \to y_0} f(x,y)$．对 $y \to y_0$ 取极限得 $|\psi(x) - A| \le \varepsilon$．\\
    由 $\varepsilon$ 的任意性知
    \begin{equation}
...
\begin{problem}{多元函数连续性}{Multivariate-Continuity}
    研究下列函数的连续性：
    \begin{enumerate}
        \item $f(x, y) = \begin{dcases} \dfrac{xy}{x-y}, & x \neq y, \\ 0, & x = y; \end{dcases}$
        \item $f(x, y) = \begin{dcases} x \sin \dfrac{1}{y}, & y \neq 0, \\ 0, & y = 0; \end{dcases}$
        \item $f(x, y) = \begin{dcases} \dfrac{x^2y}{x^2+y^2}, & (x, y) \neq (0, 0), \\ 0, & (x, y) = (0, 0); \end{dcases}$
        \item $f(x, y) = \begin{dcases} \dfrac{x-y}{x+y}, & x + y \neq 0, \\ 0, & x + y = 0. \end{dcases}$
    \end{enumerate}
\end{problem}
...
\begin{problem}{多元函数连续性与射线极限}{Directional-Continuity}
    证明：函数 $f(x, y) = \begin{dcases} \frac{x^2y}{x^4+y^2}, & x^2+y^2 > 0 \\ 0, & x^2+y^2 = 0 \end{dcases}$ 在点 $(0,0)$ 沿着过此点的每一射线 $x = t \cos \alpha, y = t \sin \alpha, (0 \le t < +\infty)$ 连续，即 $\lim_{t \to 0} f(t \cos \alpha, t \sin \alpha) = f(0,0)$. 但此函数在点 $(0,0)$ 并不连续.
\end{problem}
...
\begin{note}{伪证说明与反例分析}{Pseudo-Proof-Analysis}
    ...
    例如，考虑如下函数：
    \begin{equation}
        \boxed{f(x, y) = \begin{dcases} \frac{x^2 y}{x^4 + y^2}, & (x, y) \neq (0, 0) \\ 0, & (x, y) = (0, 0) \end{dcases}}
    \end{equation}
...
\begin{problem}{投影函数的连续性与闭性}{Projection-Continuity-Closedness}
    设 $D \subset \symbb{R}^2$，对任意 $(x, y) \in D$，令 $f(x, y) = x$．称其为 $D$ 在 $x$ 轴的投影函数．证明：投影函数是连续函数，但是它不一定将闭集映射成闭集．
\end{problem}
...
\begin{myproof}
    ...
    考虑 $\symbb{R}^2$ 中的闭集 $D = \{(x, y) \in \symbb{R}^2 \mid xy = 1, x > 0\}$．
    其在 $x$ 轴上的投影为
    \begin{equation}
        f(D) = \{x \mid (x, y) \in D\} = (0, +\infty).
    \end{equation}
    由于 $(0, +\infty)$ 不是 $\symbb{R}$ 中的闭集，故投影函数不一定将闭集映射成闭集．
\end{myproof}
