...
\begin{problem}{复合函数解析式}{Composite-Function}
    设 $f(x, y) = \begin{cases} 1, & y \ge x, \\ 0, & y < x, \end{cases}$ 又 $\begin{cases} x = \cos t, \\ y = \sin t, \end{cases}$ 求 $F(t) = f(\cos t, \sin t)$．
\end{problem}
...
\begin{solution}
    ...
    因此，函数 $F(t)$ 的表达式为：
    \begin{equation}
        \boxed{F(t) = \begin{cases} 1, & 2k\uppi + \frac{\uppi}{4} \le t \le 2k\uppi + \frac{5\uppi}{4} \\ 0, & 2k\uppi - \frac{3\uppi}{4} < t < 2k\uppi + \frac{\uppi}{4} \end{cases} \quad (k \in \symbb{Z})}
    \end{equation}
\end{solution}
