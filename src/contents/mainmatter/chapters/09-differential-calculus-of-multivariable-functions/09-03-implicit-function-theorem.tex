...
\begin{problem}{隐函数组求导}{Implicit-System-Differentiation}
    设 $x = x(z), y = y(z)$ 是方程组 $\begin{dcases} x + y + z = 0, \\ x^2 + y^2 + z^2 = 1 \end{dcases}$ 所确定的隐函数组，求 $\frac{\d x}{\d z}, \frac{\d y}{\d z}$．
\end{problem}
...
\begin{problem}{雅可比行列式}{Jacobian-Determinant-2}
    ...
    \begin{enumerate}
        \item $\begin{dcases} u^2 + v^2 + x^2 + y^2 = 1, \\ u + v + x + y = 0; \end{dcases}$
        \item $\begin{dcases} xu - yv = 0, \\ yu + xv = 1; \end{dcases}$
        \item $\begin{dcases} u = f(ux, v + y), \\ v = g(u - x, v^2y). \end{dcases}$
    \end{enumerate}
\end{problem}
...
\begin{problem}{反函数组偏导数}{Inverse-Function-Partial-Derivatives}
    ...
    \begin{enumerate}
        \item $\begin{dcases} x = f(u, v), \\ y = g(u, v); \end{dcases}$
        \item $\begin{dcases} x = \e^u + u \sin v, \\ y = \e^u - u \cos v. \end{dcases}$
    \end{enumerate}
\end{problem}
