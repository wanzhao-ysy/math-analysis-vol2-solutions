...
\begin{solution}
    ...
    在点 $M_0(a \sin \theta_0 \cos \varphi_0,\allowbreak b \sin \theta_0 \sin \varphi_0,\allowbreak c \cos \theta_0)$ 处的法向量为：
    \begin{equation}
...
\begin{problem}{直线在切平面上}{Line-On-Tangent-Plane}
    设直线 $l: \begin{dcases} x + y + b = 0, \\ x + ay - z - 3 = 0 \end{dcases}$ 在平面 $\pi$ 上，而平面 $\pi$ 与曲面 $z = x^2 + y^2$ 相切于点 $(1, -2, 5)$，求 $a, b$ 之值．
\end{problem}
...
\begin{solution}
    ...
    \end{equation}
    将其系数与 $\zcref{eq:tangent-plane}$ 对应成比例，令 $z$ 的系数相等，得 $-\lambda = -1 \implies \lambda = 1$．
    进而由其他系数对应相等得
    \begin{equation}
        \begin{dcases}
            1 + \lambda = 2,   \\
...
\begin{problem}{曲面正交}{Orthogonal-Surfaces}
    试证：曲面 $x^2 + y^2 + z^2 = ax$ 与曲面 $x^2 + y^2 + z^2 = by$ 互相正交．
\end{problem}
...
\begin{myproof}
    ...
    \begin{equation}\label{eq:dot-product-1}
        ...
        \end{aligned}
    \end{equation}
    根据曲面方程，在交点处满足 $ax = x^2 + y^2 + z^2$ 且 $by = x^2 + y^2 + z^2$．代入 \zcref{eq:dot-product-1} 得
    \begin{equation}
...
\begin{problem}{曲面相切}{Tangent-Surfaces}
    证明：曲面 $x + 2y - \ln z + 4 = 0$ 和 $x^2 - xy - 8x + z + 5 = 0$ 在点 $(2, -3, 1)$ 处相切（即有公共的切平面）．
\end{problem}
...
\begin{myproof}
    ...
    \end{equation}
    令 $X=0, Y=0$，考查 \zcref{eq:tangent-plane-eq} 右侧减左侧的差值 $\varDelta$：
    \begin{equation}
        \begin{aligned}
...
\begin{problem}{空间曲线切线与法平面}{Space-Curve-Tangent-Normal}
    求下列曲线在给定点的切线和法平面方程
    \begin{enumerate}
        \item $\begin{dcases} y^2 + z^2 = 25, \\ x^2 + y^2 = 10 \end{dcases}$ 在点 $(1, 3, 4)$；
        \item $\begin{dcases} 2x^2 + 3y^2 + z^2 = 47, \\ x^2 + 2y^2 = z \end{dcases}$ 在点 $(-2, 1, 6)$．
    \end{enumerate}
\end{problem}
...
\begin{problem}{隐函数求导证明}{Implicit-Function-Partial-Derivative}
    设方程组 $\begin{dcases} pu + qv - t^2 = 0, \\ qu + pv - s^2 = 0 \end{dcases}$ ($p^2 - q^2 \neq 0$) 确定了隐函数 $\begin{dcases} u = u(s, t), \\ v = v(s, t) \end{dcases}$ 以及反函数 $\begin{dcases} s = s(u, v), \\ t = t(u, v) \end{dcases}$．求证：
    \begin{equation}
...
\begin{myproof}
    ...
    \begin{equation}
        ...
        \end{dcases}
    \end{equation}
    由 \zcref{eq:diff-system} 解得 $\d u, \d v$
    \begin{equation}
        \begin{aligned}
    ...
    \end{equation}
    由 \zcref{eq:diff-system} 直接视 $u, v$ 为自变量，可得
    \begin{equation}
