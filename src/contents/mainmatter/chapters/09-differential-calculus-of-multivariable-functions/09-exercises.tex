...
\begin{myproof}
    ...
    \begin{equation}
        \lim_{x_n \to 0} f = \prod_{j=1}^{n-1} x_j \leqslant \left( \frac{\sum_{j=1}^{n-1} x_j}{n-1} \right)^{n-1} = \left( \frac{n}{n-1} \right)^{n-1} = \left( 1 + \frac{1}{n-1} \right)^{n-1} < \e.
    \end{equation}
    对于 $n \geqslant 3$，有 $\e < n$；对于 $n=2$，原式为 $x_1 x_2 (\frac{1}{x_1} + \frac{1}{x_2}) = x_1 + x_2 = 2$ 恒成立．
