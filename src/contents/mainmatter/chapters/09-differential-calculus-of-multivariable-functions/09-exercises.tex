...
\begin{problem}{多元函数微分学}{Partial-Derivative-Condition}
    设 $a_1, a_2, \cdots, a_n$ 是非零常数．$f(x_1, x_2, \cdots, x_n)$ 在 $\symbb{R}^n$ 上可微．求证：存在 $\symbb{R}$ 上一元可微函数 $F(s)$ 使得 $f(x_1, x_2, \cdots, x_n) = F(a_1 x_1 + a_2 x_2 + \cdots + a_n x_n)$ 的充分必要条件是 $a_j \frac{\partial f}{\partial x_i} = a_i \frac{\partial f}{\partial x_j}, i, j = 1, 2, \cdots, n$．
\end{problem}
...
\begin{solution}
    ...
    在单连通区域 $\symbb{R}^n$ 上，这意味着 $f$ 仅是 $s$ 的函数．
    即存在一元可微函数 $F$，使得
    \begin{equation}
...
\begin{myproof}
    ...
    \end{equation}
    由 \zcref{eq:differentiation-system} 后两式可得 $\frac{\partial w}{\partial x} = -\frac{w}{u} \frac{\partial u}{\partial x}$ 且 $\frac{\partial v}{\partial x} = -\frac{v}{u} \frac{\partial u}{\partial x}$．代入第一式，得
    \begin{equation}
...
\begin{myproof}
    ...
    \end{equation}
    联立 \zcref{eq:Diff-LHS} 与 \zcref{eq:Diff-RHS}，并令 $t = 1$，即得：
    \begin{equation}
        x f'_x(x, y, z) + y f'_y(x, y, z) + z f'_z(x, y, z) = k f(x, y, z).
    \end{equation}
...
\begin{myproof}
    ...
    \end{equation}
    在 \zcref{eq:homo-def} 中，令 $z = \varphi(x, y)$，并结合 \zcref{eq:implicit-relation} 可得
    \begin{equation}
    ...
    \end{equation}
    由于 $f'_z \neq 0$，根据隐函数存在定理，方程 $f(tx, ty, z) = 0$ 在该点附近确定的解 $z$ 是唯一的．通过比较 \zcref{eq:scaled-zero} 与 \zcref{eq:scaled-implicit} 可知
    \begin{equation}
...
\begin{problem}{多元函数方程}{Multivariate-Functional-Equation}
    设 $f(x,y)$ 在 $\symbb{R}^2$ 上有连续二阶偏导数，且对任意实数 $x,y,z$ 满足 $f(x,y)=f(y,x)$ 和
    \begin{equation}
...
\begin{solution}
    ...
    \end{equation}
    将 \zcref{eq:partial-x-1} 两端关于 $y$ 求偏导：
    \begin{equation}
        \frac{\partial^2 f}{\partial x \partial y}(x,y) = \frac{1}{3} g''\left( \frac{x+y+z}{3} \right). \label{eq:second-partial-1}
    \end{equation}
    由于 \zcref{eq:second-partial-1} 左端仅与 $x,y$ 相关，而右端包含变量 $z$，取 $z = -x-y$，$x$ 和 $y$ 自由变化时右侧为定值，故 $\frac{\partial^2 f}{\partial x \partial y}$ 必为常数．设该常数为 $4a$，则：
    \begin{equation}
        f(x,y) = 4axy + \varphi(x) + \psi(y).
    \end{equation}
    由 $f(x,y) = f(y,x)$ 可知 $\varphi(x) + \psi(y) = \varphi(y) + \psi(x)$，由此推得 $\varphi(x) - \psi(x) = C$．令 $h(x) = \varphi(x) - \frac{C}{2}$，则 $f(x,y)$ 可表示为：
    \begin{equation}
        f(x,y) = 4axy + h(x) + h(y).
    \end{equation}
    此时 $g(t) = f(t,t) = 4at^2 + 2h(t)$．代入 \zcref{eq:second-partial-1} 可得 $4a = \frac{1}{3}(8a + 2h''(t))$，解得 $h''(t) = 2a$．故 $h(x) = ax^2 + bx + c$．综上所述：
    \begin{equation}
...
\begin{problem}{多元函数不等式}{Multivariate-Inequality}
    证明：不等式 $\frac{x^2+y^2}{4} \le \e^{x+y-2}$ ($x \ge 0, y \ge 0$)．
\end{problem}

\begin{myproof}
    构造辅助函数
    \begin{equation}
        g(x, y) = (x^2 + y^2) \e^{2-x-y}, \quad x \ge 0, y \ge 0.
    \end{equation}
    对 $g(x, y)$ 求偏导数，可得
    \begin{equation}
        \begin{aligned}
    ...
    \end{equation}
    对称地，当 $y = 0$ 时，最大值为 $g(2, 0) = 4$．由于当 $x \to +\infty$ 或 $y \to +\infty$ 时，指数衰减速度远快于多项式增长，故 $g(x, y) \to 0$．综上可知，$g(x, y)$ 在闭区域 $x \ge 0, y \ge 0$ 上的全局最大值为 $4$．即满足
    \begin{equation}
        (x^2 + y^2) \e^{2-x-y} \le 4.
    \end{equation}
    将不等式两边同除以 $4\e^{2-x-y}$，即得
    \begin{equation}
        \boxed{\frac{x^2+y^2}{4} \le \e^{x+y-2}}.
    \end{equation}
\end{myproof}
...
\begin{myproof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    由于偏导数 $\frac{\partial f}{\partial x}$ 与 $\frac{\partial f}{\partial y}$ 在 $\symbb{R}^3$ 上连续，故在 $P_0$ 的任一紧邻域内必有界．设在该邻域内满足 $\left| \frac{\partial f}{\partial x} \right| \le M$ 且 $\left| \frac{\partial f}{\partial y} \right| \le M$，则有
    \begin{equation}
        |\Delta f| \le M|x - x_0| + M|y - y_0| + |f(x_0, y_0, z) - f(x_0, y_0, z_0)|.
    \end{equation}
...
\begin{problem}{常值函数判定}{Constant-Function-Convex-Domain}
    设 $D \subset \symbb{R}^2$ 是包含原点的凸区域，$f \in C^1(D)$．证明：若
    \begin{equation}
...
\begin{myproof}
    ...
    \end{equation}
    当 $t \in (0, 1]$ 时，由于 $(tx, ty) \in D$，利用 \zcref{eq:Euler-PDE} 可得
    \begin{equation}
...
\begin{problem}{Hadamard 引理}{Hadamard-Lemma-R2}
    设 $f \in C^1(\symbb{R}^2)$，$f(0,0) = 0$．证明：存在 $\symbb{R}^2$ 上的连续函数 $g_1, g_2$ 使得
    \begin{equation}
...
\begin{myproof}
    ...
    \end{equation}
    代入 \zcref{eq:FTC-Expansion}，并利用积分的线性性质得
    \begin{equation}
    ...
    \end{equation}
    由于 $f \in C^1(\symbb{R}^2)$，其偏导数 $\frac{\partial f}{\partial x}$ 与 $\frac{\partial f}{\partial y}$ 在 $\symbb{R}^2$ 上连续．根据含参变量积分的连续性定理可知，$g_1(x, y)$ 与 $g_2(x, y)$ 在 $\symbb{R}^2$ 上均为连续函数．由此得证
    \begin{equation}
...
\begin{myproof}
    ...
    \end{equation}
    对 \zcref{eq:increment-split} 中第一项关于变量 $x$ 使用一元拉格朗日中值定理，对第二项利用 $f_y(x_0, y_0)$ 的存在性：
    \begin{equation}
        \begin{aligned}
    ...
    \begin{equation}
        ...
        \label{eq:fx-cont}
    \end{equation}
    其中 $\beta \to 0$．代入 \zcref{eq:mvt-apply} 得
    \begin{equation}
    ...
    注意到
    \begin{equation}
        \frac{|\beta \Delta x + \alpha \Delta y|}{\sqrt{(\Delta x)^2 + (\Delta y)^2}} \le |\beta| \frac{|\Delta x|}{\rho} + |\alpha| \frac{|\Delta y|}{\rho} \le |\beta| + |\alpha| \to 0.
        \label{eq:limit-zero}
    \end{equation}
...
\begin{myproof}
    ...
    \begin{equation}
        ...
        \label{eq:vxy-pde}
    \end{equation}
    若 \zcref{eq:pde-origin} 成立，则有 $\frac{\partial^2 v}{\partial x \partial y} = 0$．积分得
    \begin{equation}
        \frac{\partial v}{\partial x} = A(x) \implies v(x, y) = \int A(x) \d x + \Psi(y) = \varphi(x) + \Psi(y).
        \label{eq:v-integral}
    \end{equation}
    从而有
    \begin{equation}
        u(x, y) = \e^{\varphi(x) + \Psi(y)} = \e^{\varphi(x)} \cdot \e^{\Psi(y)}.
        \label{eq:u-sep-form}
    \end{equation}
    取 $f(x) = \e^{\varphi(x)}$，$g(y) = \e^{\Psi(y)}$ 即证．
    ...
    \end{equation}
    两端相等，故方程 \zcref{eq:pde-origin} 成立．
\end{myproof}
...
\begin{myproof}
    ...
    \end{equation}
    将 \zcref{eq:left-side} 和 \zcref{eq:cauchy-schwarz} 代入 \zcref{eq:lagrange}，有
    \begin{equation}
...
\begin{problem}{偏导数方程}{Partial-Differential-Equation}
    设 $f(x,y,z)$ 在 $\symbb{R}^3$ 上有一阶连续偏导数，且满足 $\frac{\partial f}{\partial x} = \frac{\partial f}{\partial y} = \frac{\partial f}{\partial z}$． 如果 $f(x,0,0) > 0$ 对任意的 $x \in \symbb{R}$ 成立，求证：对任意的 $(x,y,z) \in \symbb{R}^3$，也有 $f(x,y,z) > 0$．
\end{problem}
...
\begin{myproof}
    ...
    \end{equation}
    故 $g(t)$ 在 $\symbb{R}$ 上为常值函数，则 $g(0) = g(y)$，即
    \begin{equation}
        \label{eq:step1}
        f(x,y,z) = f(x+y, 0, z).
    \end{equation}
    ...
    \end{equation}
    结合 \zcref{eq:step1} 与 \zcref{eq:step2} 可得，对任意 $(x,y,z) \in \symbb{R}^3$，均有
    \begin{equation}
        f(x,y,z) = f(x+y+z, 0, 0).
    \end{equation}
    令 $u = x+y+z \in \symbb{R}$．根据题设条件，对任意 $u \in \symbb{R}$，均有 $f(u,0,0) > 0$．因此
    \begin{equation}
        f(x,y,z) = f(u, 0, 0) > 0.
    \end{equation}
\end{myproof}
...
\begin{problem}{多元函数最值}{Multivariable-Extrema}
    求函数 $f(x,y) = x^2 + xy^2 - x$ 在区域 $D = \{(x,y) \mid x^2 + y^2 \le 2\}$ 上的最大值和最小值．
\end{problem}
...
\begin{problem}{Lagrange 乘数法}{Lagrange-Multipliers-Inequality}
    ...
    \begin{equation}
        x_1 x_2 \dots x_n \left( \frac{1}{x_1} + \frac{1}{x_2} + \dots + \frac{1}{x_n} \right) \le n,
    \end{equation}
...
\begin{myproof}
    ...
    令目标函数与约束条件分别为
    \begin{equation}
        f(x_1, x_2, \dots, x_n) = \sum_{i=1}^n \frac{\prod_{j=1}^n x_j}{x_i}, \quad g(x_1, x_2, \dots, x_n) = \sum_{i=1}^n x_i - n = 0.
    \end{equation}
    记 $P = \prod_{j=1}^n x_j$，则 $f = \sum_{i=1}^n \frac{P}{x_i}$．构造 Lagrange 函数：
    \begin{equation}
        L(x_1, x_2, \dots, x_n, \lambda) = f(x_1, x_2, \dots, x_n) + \lambda \left( \sum_{i=1}^n x_i - n \right).
    \end{equation}
    对 $x_k$ ($k = 1, 2, \dots, n$) 求偏导数．注意到 $\frac{\partial P}{\partial x_k} = \frac{P}{x_k}$，故有
    \begin{equation}\label{eq:partial-f}
        \frac{\partial f}{\partial x_k} = \frac{\partial}{\partial x_k} \left( \frac{P}{x_k} + \sum_{i \neq k} \frac{P}{x_i} \right) = 0 + \sum_{i \neq k} \frac{P}{x_i x_k} = \frac{1}{x_k} \left( f - \frac{P}{x_k} \right).
    \end{equation}
    令 $\frac{\partial L}{\partial x_k} = 0$，得方程组
    \begin{equation}\label{eq:stationary-cond}
        \frac{1}{x_k} \left( f - \frac{P}{x_k} \right) + \lambda = 0 \implies \lambda x_k = \frac{P}{x_k} - f.
    \end{equation}
    对 \zcref{eq:stationary-cond} 关于 $k$ 从 $1$ 到 $n$ 求和，利用 $\sum_{k=1}^n x_k = n$ 和 $\sum_{k=1}^n \frac{P}{x_k} = f$，得
    \begin{equation}
        \lambda n = f - nf \implies \lambda = \frac{1-n}{n} f.
    \end{equation}
    将 $\lambda$ 代回 \zcref{eq:stationary-cond}，整理得关于 $x_k$ 的二次方程：
    \begin{equation}\label{eq:quadratic-res}
        \frac{n-1}{n} f x_k^2 - f x_k + P = 0.
    \end{equation}
    由于该二次方程对所有 $k$ 形式相同且至多有两个正根，结合对称性及约束条件 $\sum x_i = n$，考察所有变量相等的情况．令 $x_1 = x_2 = \dots = x_n = 1$，代入 \zcref{eq:quadratic-res} 验证：
    \begin{equation}
        \frac{n-1}{n} \cdot n \cdot (1)^2 - n \cdot (1) + 1 = (n-1) - n + 1 = 0.
    \end{equation}
    此时驻点处函数值为 $f(1, 1, \dots, 1) = n$．
    若定义域边界存在 $x_i \to 0$，则 $P \to 0$．由算术--几何平均值不等式可知，当其余 $n-1$ 个变量趋于一致时 $f$ 取边界极大值：
    \begin{equation}
        \lim_{x_n \to 0} f = \prod_{j=1}^{n-1} x_j \le \left( \frac{\sum_{j=1}^{n-1} x_j}{n-1} \right)^{n-1} = \left( \frac{n}{n-1} \right)^{n-1} = \left( 1 + \frac{1}{n-1} \right)^{n-1} < \e.
    \end{equation}
    对于 $n \ge 3$，有 $\e < n$；对于 $n=2$，原式为 $x_1 x_2 (\frac{1}{x_1} + \frac{1}{x_2}) = x_1 + x_2 = 2$ 恒成立．
    因此，$f$ 在 $(1, 1, \dots, 1)$ 处取得最大值 $n$．

    \begin{equation}
        \boxed{x_1 x_2 \dots x_n \left( \frac{1}{x_1} + \frac{1}{x_2} + \dots + \frac{1}{x_n} \right) \le n}
    \end{equation}
\end{myproof}
...
\begin{myproof}
    ...
    \begin{equation}
        \frac{a_1 + \cdots + a_n}{n} \le \left( \frac{a_1^p + \cdots + a_n^p}{n} \right)^{1/p}.
    \end{equation}
    由于 $f(x)$ 为严格凸函数，根据 \zcref{eq:jensen-apply}，等号成立的充分必要条件为
    \begin{equation}
...
\begin{myproof}
    ...
    将 \zcref{eq:diff-definition-1} 代入左端的外积表达式中，利用外积的线性性质，得
    \begin{equation}
        \begin{aligned}
