    ...
    \begin{equation}
        \begin{aligned}
            \text{原式} & = \int_{0}^{2\uppi} \d \theta \int_{0}^{R} r \d r \int_{0}^{\sqrt{R^{2}-r^{2}}} r^{2} \d z \\
                      & = 2\uppi \int_{0}^{R} r^{3} \sqrt{R^{2}-r^{2}} \d r
        \end{aligned}
    \end{equation}
...
\begin{problem}{三重积分计算}{Triple-Integrals}
    ...
    \begin{enumerate}
        ...
        \item $\iiint_{V} x y z \d x \d y \d z$，其中 $V$ 是 $x^{2}+y^{2}+z^{2} \le 1$ 的第一卦限部分；
        \item $\iiint_{V} x^{2} \d x \d y \d z$，其中 $V$ 是由曲面 $z=y^{2}$、$z=4 y^{2}(y>0)$ 及平面 $z=x$、$z=2 x$、$z=1$ 所围成的区域；
        \item $\iiint_{V}\left|x^{2}+y^{2}+z^{2}-1\right| \d x \d y \d z$，其中 $V: x^{2}+y^{2}+z^{2} \le 4$；
        \item $\iiint_{V} \e^{|z|} \d x \d y \d z$，其中 $V: x^{2}+y^{2}+z^{2} \le 1$；
        \item $\iiint_{V}(|x|+z) \e^{-\left(x^{2}+y^{2}+z^{2}\right)} \d x \d y \d z$，其中 $V: 1 \le x^{2}+y^{2}+z^{2} \le 4$．
    \end{enumerate}
\end{problem}

\begin{solution}
    \ansitem{1}
    采用柱面坐标，令 $x=\rho \cos \theta$、$y=\rho \sin \theta$、$z=z$．积分区域 $V$ 可表示为 $0 \le \theta \le 2 \uppi$、$0 \le z \le 2$、$0 \le \rho \le \sqrt{2 z}$．
    ...
    \ansitem{2}
    采用柱面坐标，积分区域 $V$ 可表示为 $0 \le \theta \le 2 \uppi$、$0 \le z \le 1$、$0 \le \rho \le z$．
    ...
    \ansitem{3}
    联立方程 $\sqrt{4-x^{2}-y^{2}}=z$ 与 $x^{2}+y^{2}=3 z$．由于 $z \ge 0$，解得交线位于 $z=1$ 平面处．积分区域在 $xy$ 平面上的投影为圆盘 $D: x^2+y^2 \le 3$．
    ...
    \ansitem{4}
    采用球坐标，积分区域 $V$ 对应的参数范围为 $0 \le r \le 1$、$0 \le \theta \le \frac{\uppi}{2}$、$0 \le \varphi \le \frac{\uppi}{2}$．
...
\begin{problem}{三重积分的对称性与坐标变换}{Triple-Integrals-Symmetry-And-Transformation}
    ...
    \begin{enumerate}
        ...
        \item $\iiint_{V} \sqrt{x^{2}+y^{2}+z^{2}} \d x \d y \d z, V: x^{2}+y^{2}+z^{2} \le x$；
        \item $\iiint_{V}\left(x^{2}+y^{2}\right) \d x \d y \d z, V: r^{2} \le x^{2}+y^{2}+z^{2} \le R^{2}, z \ge 0$；
        \item $\iiint_{V} \sqrt{1-\frac{x^{2}}{a^{2}}-\frac{y^{2}}{b^{2}}-\frac{z^{2}}{c^{2}}} \d x \d y \d z, V: \frac{x^{2}}{a^{2}}+\frac{y^{2}}{b^{2}}+\frac{z^{2}}{c^{2}} \le 1$；
        \item $\iiint_{V} \frac{z \ln \left(x^{2}+y^{2}+z^{2}+1\right)}{x^{2}+y^{2}+z^{2}+1} \d x \d y \d z, V: x^{2}+y^{2}+z^{2} \le 1$．
    \end{enumerate}
\end{problem}
...
\begin{problem}{三重积分平均值}{Average-Value-Triple-Integral}
    求函数 $f(x, y, z) = x^2 + y^2 + z^2$ 在域 $x^2 + y^2 + z^2 \le x + y + z$ 内的平均值．
\end{problem}

\begin{solution}
    设域 $\Omega$ 为 $x^2 + y^2 + z^2 \le x + y + z$．将其配方可得
    \begin{equation}\label{eq:Domain-Center-1}
        \left( x - \frac{1}{2} \right)^2 + \left( y - \frac{1}{2} \right)^2 + \left( z - \frac{1}{2} \right)^2 \le \frac{3}{4}.
    \end{equation}
    ...
    \end{equation}
    作变量代换 $u = x - \frac{1}{2}$、$v = y - \frac{1}{2}$、$w = z - \frac{1}{2}$，则 $\Omega$ 变换为原点处的球域 $\Omega': u^2 + v^2 + w^2 \le \frac{3}{4}$．被积函数变为
...
\begin{problem}{二重积分的应用}{Ellipse-Plate-Mass}
    求椭圆薄片 $\frac{x^2}{a^2} + \frac{y^2}{b^2} \le 1$ 的质量，设密度 $\rho = \frac{x^2}{a^2} + \frac{y^2}{b^2}$．
\end{problem}

\begin{solution}
    设椭圆域为 $D$．引入广义极坐标变换，令 $x = ar \cos \theta$，$y = br \sin \theta$，其雅可比行列式绝对值为 $J = abr$．此时积分区域 $D$ 映射为 $D' = \{(r, \theta) \mid 0 \le r \le 1, 0 \le \theta \le 2\uppi\}$．质量 $M$ 的计算过程如下：
...
\begin{problem}{重心坐标计算}{Center-of-Mass-Sphere}
    设球体 $x^2 + y^2 + z^2 \le 2az$ 内各点密度与各点到原点的距离成反比，求其重心坐标．
\end{problem}
...
\begin{solution}
    ...
    \begin{equation}
        ...
        \end{cases}
    \end{equation}
    球体方程化为 $r \le 2a \cos \varphi$，积分区域可表示为 $0 \le \theta \le 2\uppi$、$0 \le \varphi \le \frac{\uppi}{2}$、$0 \le r \le 2a \cos \varphi$．
