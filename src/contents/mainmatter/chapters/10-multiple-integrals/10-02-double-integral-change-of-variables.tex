...
\begin{problem}{二重积分的计算}{Double-Integral-Calculation}
    计算下列二重积分：
    \begin{enumerate}
        \item $\iint_{D} \sqrt{x^{2}+y^{2}} \d x \d y$，其中 $D: x^{2}+y^{2} \le x+y$；
        ...
        \item $\iint_{D} 4xy \d x \d y$，其中 $D: x^{4}+y^{4} \le 1, x \ge 0, y \ge 0$ 围成的区域；
        \item $\iint_{D} \frac{x^{2}-y^{2}}{\sqrt{x+y+3}} \d x \d y$，其中 $D: |x|+|y| \le 1$；
        \item $\iint_{D} \sin \frac{y}{x+y} \d x \d y$，其中 $D$ 为由直线 $x+y=1, x=0, y=0$ 围成的区域；
        \item $\iint_{D} |xy| \d x \d y$，其中 $D = \{(x,y) : x^2 + y^2 \le a^2\}$．
    \end{enumerate}
\end{problem}
...
\begin{solution}
    ...
    利用极坐标变换 $x = r \cos \theta$、$y = r \sin \theta$，区域 $D$ 的边界方程为 $r^2 \le r(\cos \theta + \sin \theta)$，即 $r \le \sqrt{2} \cos(\theta - \uppi/4)$，对应的辐角范围为 $\theta \in [-\uppi/4, 3\uppi/4]$．积分计算如下：
    \begin{equation}
        \begin{aligned}
            \iint_D \sqrt{x^2+y^2} \d x \d y & = \int_{-\uppi/4}^{3\uppi/4} \d \theta \int_0^{\sqrt{2}\cos(\theta - \uppi/4)} r^2 \d r                   \\
                                             & = \frac{1}{3} \int_{-\uppi/2}^{\uppi/2} (\sqrt{2}\cos \varphi)^3 \d \varphi \label{eq:int-1-step}         \\
                                             & = \frac{4\sqrt{2}}{3} \int_0^{\uppi/2} \cos^3 \varphi \d \varphi = \frac{4\sqrt{2}}{3} \cdot \frac{2}{3}.
        \end{aligned}
    \end{equation}
    \begin{equation}
    ...
    积分区域 $D: |x|+|y| \le 1$ 关于直线 $y=x$ 对称．将被积函数中的 $x$ 与 $y$ 对换，根据对称性有
    \begin{equation}
    ...
    引入变换 $u = x+y$、$v = y$，则 $\d x \d y = \d u \d v$．区域 $D$ 映射为 $0 \le u \le 1$，$0 \le v \le u$．积分如下：
    \begin{equation}
        \begin{aligned}
...
\begin{solution}
    ...
    计算该变换的雅可比行列式：
    \begin{equation}
        J = \frac{\partial(x, y)}{\partial(u, v)} = \det \begin{bmatrix} \frac{1}{1+v} & -\frac{u}{(1+v)^2} \\ \frac{v}{1+v} & \frac{u}{(1+v)^2} \end{bmatrix} = \frac{u}{(1+v)^2}.
    \end{equation}
...
\begin{myproof}
    ...
    \begin{equation}
        \begin{dcases} u = x + y \\ v = x - y \end{dcases} \implies \begin{dcases} x = \frac{1}{2}(u + v) \\ y = \frac{1}{2}(u - v) \end{dcases},
    \end{equation}
