...
\begin{solution}
    ...
    设长方体围成的闭区域为 $\Omega$，其整个外侧表面为 $S$。根据 Gauss 公式，向量场 $\symbfit{v}$ 穿过闭曲面 $S$ 向外的通量 $\Phi$ 可以表示为三重积分
    \begin{equation}
        \Phi = \iint_S \symbfit{v} \cdot \d \symbfit{S} = \iiint_{\Omega} \operatorname{div} \symbfit{v} \d V. \label{eq:gauss-1}
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            \Phi & = \iiint_{\Omega} (x^2 + 1) \d x \d y \d z             \\
    ...
    因此，所求的通量为
    \begin{equation}
        \Phi = \boxed{abc \left( \frac{a^2}{3} + 1 \right)}.
    \end{equation}
\end{solution}
