...
\begin{solution}
    ...
    \end{equation}
    \zcref{eq:ode-standard} 两边同乘积分因子 $\mu(x)$，得
    \begin{equation}
...
\begin{myproof}
    ...
    对于等 \zcref{eq:gauss-app} 的左端，由于被积函数仅与 $x,y$ 有关，且柱体高为 $1$（即 $0 \le z \le 1$），化为累次积分可得
    \begin{equation}
        \label{eq:lhs-eval}
        \begin{aligned}
    ...
    对于等 \zcref{eq:gauss-app} 的右端，将全表面分为底面 $S_1$、顶面 $S_2$ 和侧面 $S_3$．
