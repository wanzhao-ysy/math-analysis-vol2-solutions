...
\begin{solution}
    ...
    \end{equation}
    此时，积分区域 $V$ 可表示为 $0 \le \phi \le 2\uppi$、$0 \le \theta \le \frac{\uppi}{2}$、$0 \le r \le \cos \theta$。从而有
    \begin{equation}
        \label{eq:part4-3}
        \begin{aligned}
            I & = \int_0^{2\uppi} \d \phi \int_0^{\uppi/2} \d \theta \int_0^{\cos \theta} r^4 \sin \theta \d r \\
...
\begin{solution}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    设闭曲面 $S$ 包围原点，取以原点为球心、$\epsilon > 0$ 为半径的足够小球面 $S_\epsilon = \{ (x, y, z) \mid x^2 + y^2 + z^2 = \epsilon^2 \}$，使得 $S_\epsilon$ 被包围在 $S$ 内部。记由 $S$ 与 $S_\epsilon$ 围成的闭区域为 $V_\epsilon$。由于在 $V_\epsilon$ 上 $\operatorname{div} \symbfit{F} = 0$，根据高斯公式可得：
    \begin{equation}
        \label{eq:gauss-1-1}
        \iint_{S} \symbfit{F} \cdot \d \symbfit{S} - \iint_{S_\epsilon} \symbfit{F} \cdot \d \symbfit{S} = \iiint_{V_\epsilon} \operatorname{div} \symbfit{F} \d V = 0.
    \end{equation}
    由此可知：
    \begin{equation}
        \label{eq:relation-1}
        \iint_{S} \symbfit{F} \cdot \d \symbfit{S} = \iint_{S_\epsilon} \symbfit{F} \cdot \d \symbfit{S}.
    \end{equation}
    在 $S_\epsilon$ 上，外法线单位向量为 $\symbfit{n} = \frac{\symbfit{r}}{\epsilon}$，此时有：
    \begin{equation}
        \label{eq:integrand-1}
        \symbfit{F} \cdot \symbfit{n} = -km \frac{\symbfit{r}}{\epsilon^3} \cdot \frac{\symbfit{r}}{\epsilon} = -\frac{km}{\epsilon^2}.
    \end{equation}
    因而通量计算为：
    \begin{equation}
        \label{eq:flux-calc-1}
        \iint_{S_\epsilon} \symbfit{F} \cdot \d \symbfit{S} = \iint_{S_\epsilon} \left( -\frac{km}{\epsilon^2} \right) \d S = -\frac{km}{\epsilon^2} (4\uppi \epsilon^2) = -4\uppi km.
    \end{equation}
    最终结果为：
    \begin{equation}
    ...
    \begin{equation}
        \label{eq:gauss-2}
        \Phi = \iint_S \symbfit{F} \cdot \d \symbfit{S} = \iiint_V \operatorname{div} \symbfit{F} \d V = 0.
    \end{equation}
    最终结果为：
    \begin{equation}
    ...
    由于原点位于光滑的闭曲面 $S$ 上，取以原点为球心、$\epsilon > 0$ 为半径的小球 $B_\epsilon$，其边界为 $S_\epsilon$。记曲面 $S$ 在 $B_\epsilon$ 外部的部分为 $S \setminus B_\epsilon$，而 $S_\epsilon$ 落在 $S$ 内部的球面部分为 $S_{\epsilon, \text{in}}$。记 $S$ 与 $S_{\epsilon, \text{in}}$ 共同围成的区域为 $V_\epsilon$，此时原点在 $V_\epsilon$ 外部。根据高斯公式有：
    \begin{equation}
        \label{eq:gauss-3}
        \iint_{S \setminus B_\epsilon} \symbfit{F} \cdot \d \symbfit{S} - \iint_{S_{\epsilon, \text{in}}} \symbfit{F} \cdot \symbfit{n} \d S = \iiint_{V_\epsilon} \operatorname{div} \symbfit{F} \d V = 0.
    \end{equation}
    其中 $\symbfit{n}$ 为 $S_\epsilon$ 的外法线向量。因此有：
    \begin{equation}
        \label{eq:relation-3}
        \iint_{S \setminus B_\epsilon} \symbfit{F} \cdot \d \symbfit{S} = \iint_{S_{\epsilon, \text{in}}} \symbfit{F} \cdot \symbfit{n} \d S.
    \end{equation}
    由于 $S$ 在原点处是光滑的，当 $\epsilon \to 0^+$ 时，$S_{\epsilon, \text{in}}$ 趋近于一个半球面，其立体角趋近于 $2\uppi$。因此，该部分球面面积满足：
    \begin{equation}
        \label{eq:area-3}
        \operatorname{Area}(S_{\epsilon, \text{in}}) \sim 2\uppi \epsilon^2,
        \quad \epsilon \to 0^+.
    \end{equation}
    代入 $\symbfit{F} \cdot \symbfit{n} = -\frac{km}{\epsilon^2}$，取极限 $\epsilon \to 0^+$ 可得：
    \begin{equation}
        \label{eq:limit-3}
        \begin{aligned}
            \Phi & = \lim_{\epsilon \to 0^+} \iint_{S \setminus B_\epsilon} \symbfit{F} \cdot \d \symbfit{S}                          \\
                 & = \lim_{\epsilon \to 0^+} \iint_{S_{\epsilon, \text{in}}} \left( -\frac{km}{\epsilon^2} \right) \d S               \\
                 & = \lim_{\epsilon \to 0^+} \left[ -\frac{km}{\epsilon^2} \cdot \operatorname{Area}(S_{\epsilon, \text{in}}) \right] \\
                 & = -2\uppi km.
        \end{aligned}
    \end{equation}
    最终结果为：
...
\begin{solution}
    ...
    由于原点 $O$ 是该向量场的奇点，我们在区域 $V$ 内部取一个以原点为球心、半径为 $\epsilon > 0$ 且足够小的球面 $S_\epsilon \colon x^2+y^2+z^2 = \epsilon^2$，并取其外侧。记由 $S$ 与 $S_\epsilon$ 所围成的闭区域为 $V_\epsilon$。在 $V_\epsilon$ 上，向量场 $\symbfit{V}$ 连续可微且散度处处为零。

    根据高斯公式，有
    \begin{equation}
        \label{eq:gauss-theorem}
        \iint_S \symbfit{V} \cdot \d\symbfit{S} - \iint_{S_\epsilon} \symbfit{V} \cdot \d\symbfit{S} = \iiint_{V_\epsilon} \operatorname{div} \symbfit{V} \d V = 0.
    \end{equation}
    由此可得
    \begin{equation}
        \label{eq:equal-integrals}
        I = \iint_{S} \symbfit{V} \cdot \d\symbfit{S} = \iint_{S_\epsilon} \symbfit{V} \cdot \d\symbfit{S}.
    \end{equation}

    在球面 $S_\epsilon$ 上，外法线单位向量为 $\symbfit{n} = \frac{\symbfit{r}}{\epsilon}$，且在曲面上 $r = \epsilon$。计算向量场与法线向量的点积：
    \begin{equation}
        \label{eq:integrand-eval}
        \symbfit{V} \cdot \symbfit{n} = \frac{\symbfit{r}}{\epsilon^3} \cdot \frac{\symbfit{r}}{\epsilon} = \frac{r^2}{\epsilon^4} = \frac{\epsilon^2}{\epsilon^4} = \frac{1}{\epsilon^2}.
    \end{equation}
    将 \zcref{eq:integrand-eval} 代入 \zcref{eq:equal-integrals} 中进行积分，即可得到
    \begin{equation}
        \label{eq:final-eval}
        I = \iint_{S_\epsilon} \frac{1}{\epsilon^2} \d S = \frac{1}{\epsilon^2} \operatorname{Area}(S_\epsilon) = \frac{1}{\epsilon^2} \left(4\uppi \epsilon^2\right) = 4\uppi.
    \end{equation}
