...
\begin{solution}
    ...
    \end{equation}
    其中 $\d \symbfit{S} = \symbfit{n} \d S$ 为外法向面积微元．在球坐标系 $(r, \theta, \varphi)$ 中，选取由坐标面构成的微小体积元，边界范围分别为 $r$ 到 $r+\Delta r$、$\theta$ 到 $\theta+\Delta \theta$、$\varphi$ 到 $\varphi+\Delta \varphi$，其体积可近似表示为：
    \begin{equation}
        \Delta V \approx r^2 \sin\theta \Delta r \Delta \theta \Delta \varphi.
    \end{equation}

    我们将闭合曲面积分拆解为径向（$r$）、极角（$\theta$）和方位角（$\varphi$）三对相对面上的面积分近似之和．在径向的两个面上，面积微元大小约为 $r^2 \sin\theta \Delta \theta \Delta \varphi$，外法向量为 $\pm \symbfit{e}_r$，其积分贡献为：
    \begin{equation}
        \begin{aligned}
            I_r & \approx \left( f(r+\Delta r) \symbfit{e}_r(r+\Delta r)^2 - f(r) \symbfit{e}_r r^2 \right) \sin\theta \Delta \theta \Delta \varphi \\
                & \approx \frac{\partial}{\partial r} (f r^2 \symbfit{e}_r) \sin\theta \Delta r \Delta \theta \Delta \varphi.
        \end{aligned}
    \end{equation}

    在极角的两个面上，面积微元大小约为 $r \sin\theta \Delta r \Delta \varphi$，外法向量为 $\pm \symbfit{e}_\theta$，积分贡献为：
    \begin{equation}
        I_\theta \approx \frac{\partial}{\partial \theta} (f \sin\theta \symbfit{e}_\theta) r \Delta r \Delta \theta \Delta \varphi.
    \end{equation}

    在方位角的两个面上，面积微元大小约为 $r \Delta r \Delta \theta$，外法向量为 $\pm \symbfit{e}_\varphi$，积分贡献为：
    \begin{equation}
        I_\varphi \approx \frac{\partial}{\partial \varphi} (f \symbfit{e}_\varphi) r \Delta r \Delta \theta \Delta \varphi.
    \end{equation}

    在球坐标系中，局部基向量对各坐标变量的偏导数满足如下几何关系：
    \begin{equation}
        \label{eq:base-vector-deriv}
        \frac{\partial \symbfit{e}_r}{\partial r} = \symbfit{0}, \quad \frac{\partial \symbfit{e}_\theta}{\partial \theta} = -\symbfit{e}_r, \quad \frac{\partial \symbfit{e}_\varphi}{\partial \varphi} = -\sin\theta \symbfit{e}_r - \cos\theta \symbfit{e}_\theta.
    \end{equation}

    利用\zcref{eq:base-vector-deriv}中基向量的导数公式，利用乘积法则分别展开 $I_r$、$I_\theta$、$I_\varphi$ 中的偏导数：
    \begin{equation}
        \begin{aligned}
            I_r       & \approx \symbfit{e}_r \sin\theta \left( r^2 \frac{\partial f}{\partial r} + 2r f \right) \Delta r \Delta \theta \Delta \varphi,                                                                                                      \\
            I_\theta  & \approx r \left( \frac{\partial f}{\partial \theta} \symbfit{e}_\theta \sin\theta + f \frac{\partial \symbfit{e}_\theta}{\partial \theta} \sin\theta + f \symbfit{e}_\theta \cos\theta \right) \Delta r \Delta \theta \Delta \varphi \\
                      & = r \left( \frac{\partial f}{\partial \theta} \symbfit{e}_\theta \sin\theta - f \symbfit{e}_r \sin\theta + f \symbfit{e}_\theta \cos\theta \right) \Delta r \Delta \theta \Delta \varphi,                                            \\
            I_\varphi & \approx r \left( \frac{\partial f}{\partial \varphi} \symbfit{e}_\varphi + f \frac{\partial \symbfit{e}_\varphi}{\partial \varphi} \right) \Delta r \Delta \theta \Delta \varphi                                                     \\
                      & = r \left( \frac{\partial f}{\partial \varphi} \symbfit{e}_\varphi - f \sin\theta \symbfit{e}_r - f \cos\theta \symbfit{e}_\theta \right) \Delta r \Delta \theta \Delta \varphi.
        \end{aligned}
    \end{equation}

    将上述三部分积分相加，所有包含纯代数 $f$ 本身的项恰好相互抵消：
    \begin{equation}
        \oiint_{S} f \d \symbfit{S} = I_r + I_\theta + I_\varphi \approx \left( r^2 \sin\theta \frac{\partial f}{\partial r} \symbfit{e}_r + r \sin\theta \frac{\partial f}{\partial \theta} \symbfit{e}_\theta + r \frac{\partial f}{\partial \varphi} \symbfit{e}_\varphi \right) \Delta r \Delta \theta \Delta \varphi.
    \end{equation}

    将其代入梯度的积分表示中，并除以体积微元 $\Delta V = r^2 \sin\theta \Delta r \Delta \theta \Delta \varphi$，最终推导出球坐标系下的梯度公式：
    \begin{equation}
        \boxed{\operatorname{grad} f = \frac{\partial f}{\partial r} \symbfit{e}_r + \frac{1}{r} \frac{\partial f}{\partial \theta} \symbfit{e}_\theta + \frac{1}{r \sin\theta} \frac{\partial f}{\partial \varphi} \symbfit{e}_\varphi}
    \end{equation}
\end{solution}
...
\begin{myproof}
    ...
    利用 \zcref{eq:directional-derivative-1} 以及高斯公式（散度定理），将面积分转化为体积分：
    \begin{equation}
        \begin{aligned}
    ...
    将被积函数中的方向导数使用 \zcref{eq:directional-derivative-1} 展开，并将 $u \nabla u$ 视作一个整体的向量场，应用高斯公式：
    \begin{equation}
