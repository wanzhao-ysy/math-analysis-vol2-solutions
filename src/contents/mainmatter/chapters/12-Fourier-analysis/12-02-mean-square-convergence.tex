...
\begin{problem}{Parseval等式与级数求和}{Parseval-Series-Sum}
    将 $f(x) = \begin{cases} 1, & |x| < a, \\ 0, & a \leqslant |x| < \uppi \end{cases}$ 展开成 Fourier 级数，然后利用 Parseval 等式求下列级数的和：
    \begin{enumerate}
...
\begin{solution}
    ...
    对\zcref{eq:subs-5}式两边同乘以 $\frac{\uppi^2}{4}$ 并移项，可得
...
\begin{problem}{Fourier级数计算与求和}{Fourier-Series-Summation}
    求周期为 $2\uppi$ 函数 $f(x) = \begin{cases} -1, & -\uppi < x < 0, \\ 1, & 0 \leqslant x \leqslant \uppi \end{cases}$ 的 Fourier 级数，并求 $\sum_{n=1}^{\infty} \frac{1}{(2n-1)^2}$ 的和以及 $\sum_{n=1}^{\infty} \frac{\cos(2n-1)x}{(2n-1)^2} \quad (0 \leqslant x \leqslant \uppi)$ 的和。
\end{problem}
...
\begin{solution}
    ...
    \begin{equation}\label{eq:bn-cases-3}
        b_n = \begin{cases} \frac{4}{n\uppi}, & n = 2k-1, \\ 0, & n = 2k. \end{cases}
    \end{equation}
    ...
    \begin{equation}\label{eq:term-integration-3}
        ...
        \end{aligned}
    \end{equation}
    将前面求得的 $\sum_{n=1}^{\infty} \frac{1}{(2n-1)^2} = \frac{\uppi^2}{8}$ 代入\zcref{eq:term-integration-3}式，得
    \begin{equation}\label{eq:fx-subs-3}
        |x| = \frac{\uppi}{2} - \frac{4}{\uppi} \sum_{n=1}^{\infty} \frac{\cos(2n-1)x}{(2n-1)^2}.
    \end{equation}
    当 $0 \leqslant x \leqslant \uppi$ 时，有 $|x| = x$，将其代入\zcref{eq:fx-subs-3}式移项整理，即得第二个级数的和为
    \begin{equation}
...
\begin{myproof}
    ...
    设 $\phi_0(x) = 1$，$\phi_n(x) = \cos(nx)$ ($n \geqslant 1$)。
    ...
    \begin{equation}\label{eq:norm-1-1}
        \|\phi_0\|^2 = \int_{0}^{\uppi} 1 \d x = \uppi,
    \end{equation}
    \begin{equation}\label{eq:norm-1-2}
        \|\phi_n\|^2 = \int_{0}^{\uppi} \cos^2(nx) \d x = \frac{\uppi}{2} \quad (n \geqslant 1).
    \end{equation}
    对应的标准正交系为
    \begin{equation}
    ...
    设 $\phi_n(x) = \sin \frac{n\uppi x}{l}$ ($n \geqslant 1$)。
    ...
    \begin{equation}\label{eq:norm-2}
        \|\phi_n\|^2 = \int_{0}^{l} \sin^2 \frac{n\uppi x}{l} \d x = \frac{l}{2}.
    \end{equation}
    对应的标准正交系为
    \begin{equation}
    ...
    设 $\phi_n(x) = \sin(2n+1)x$ ($n \geqslant 0$)。
    ...
    \begin{equation}\label{eq:norm-3}
        \|\phi_n\|^2 = \int_{0}^{\uppi/2} \sin^2(2n+1)x \d x = \frac{1}{2} \int_{0}^{\uppi/2} [1 - \cos 2(2n+1)x] \d x = \frac{\uppi}{4}.
    \end{equation}
    对应的标准正交系为
    \begin{equation}
    ...
    设 $\phi_n(x) = \cos \frac{(2n+1)\uppi x}{2l}$ ($n \geqslant 0$)。
    对于 $p \neq q$ ($p, q \geqslant 0$)，有
    ...
    \begin{equation}\label{eq:norm-4}
        \|\phi_n\|^2 = \int_{0}^{l} \cos^2 \frac{(2n+1)\uppi x}{2l} \d x = \frac{l}{2}.
    \end{equation}
    对应的标准正交系为
    \begin{equation}
...
\begin{myproof}
    ...
    \end{equation}
    对\zcref{eq:integral-step1-7}式右端进行分部积分，得
...
\begin{myproof}
    ...
    \begin{equation}\label{eq:diff-eq-u-8}
        (x^2 - 1) u'(x) = 2nx u(x).
    \end{equation}
    对\zcref{eq:diff-eq-u-8}式关于 $x$ 求 $n+1$ 阶导数。利用 Leibniz 导数法则，等式左端展开为
    ...
    \end{equation}
    将\zcref{eq:lemma1-step3}式代回\zcref{eq:lemma1-step1}式中，可得
    ...
    \begin{equation}\label{eq:lemma2-step4}
        ...
        \end{aligned}
    \end{equation}
    移项即得引理 2，且这正是公式\zcref{eq:recurrence-8c}。

    下面基于引理 1 与引理 2 验证其余两式。

    (a) 验证公式\zcref{eq:recurrence-8c}

    由引理 2，公式\zcref{eq:recurrence-8c}直接成立。

    (b) 验证公式\zcref{eq:recurrence-8b}
    ...
    \end{equation}
    联立\zcref{eq:proof-b-1}与\zcref{eq:proof-b-2}式以消去 $P'_{n+1}(x)$，可得
    ...
    \end{equation}
    将\zcref{eq:proof-b-3}式代入\zcref{eq:proof-b-4}式以消去 $P'_{n-1}(x)$，得
    ...
    \end{equation}
    这验证了公式\zcref{eq:recurrence-8b}。

    (c) 验证公式\zcref{eq:recurrence-8a}

    由已证的公式\zcref{eq:recurrence-8b}，对任意正整数 $k$ 均有
    ...
    \end{equation}
    将 $k = n$ 时的\zcref{eq:proof-a-1}式代入\zcref{eq:proof-a-3}式右端，得
    \begin{equation}\label{eq:proof-a-4}
        ...
        \end{aligned}
    \end{equation}
    比较\zcref{eq:proof-a-2}与\zcref{eq:proof-a-4}式的右端，有
