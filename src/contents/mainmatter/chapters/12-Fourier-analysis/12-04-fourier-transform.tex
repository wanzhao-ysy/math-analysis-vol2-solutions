...
\begin{solution}
    ...
    \begin{equation}\label{eq:p2s3-int}
        ...
        \end{aligned}
    \end{equation}
    当 $\omega = \pm 1$ 时，由极限或直接积分可得 $F(\pm 1) = \frac{\uppi}{2}$，这与\zcref{eq:p2s3-int}式的极限值一致。因此
    \begin{equation}
...
\begin{solution}
    ...
    \end{equation}
    由于虚部被积函数 $\frac{\sin\omega \sin(\omega x)}{\omega}$ 关于 $\omega$ 是奇函数，在对称区间上的积分为 $0$。而实部被积函数为偶函数，故\zcref{eq:sub-4}式可化简为
    ...
    \end{enumerate}
    将上述讨论代入\zcref{eq:alpha-int-6}式，即证
    \begin{equation}
