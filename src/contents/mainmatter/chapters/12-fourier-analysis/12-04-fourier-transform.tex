...
\begin{problem}{Fourier积分表示}{Fourier-Integral-Representation}
    用 Fourier 积分表示下列函数：
    \begin{enumerate}
        \item $f(x) = \begin{dcases} 0, & x \leqslant 0, \\ kx, & 0 < x < T, \\ 0, & x \geqslant T; \end{dcases}$
        \item $f(x) = \begin{dcases} \operatorname{sgn} x, & |x| \leqslant 1, \\ 0, & |x| > 1; \end{dcases}$
...
\begin{problem}{Fourier变换}{Fourier-Transform}
    ...
    \begin{enumerate}
        ...
        \item $f(x) = \begin{dcases} \cos x, & |x| \leqslant \frac{\uppi}{2}, \\ 0, & |x| > \frac{\uppi}{2}. \end{dcases}$
    \end{enumerate}
\end{problem}
...
\begin{solution}
    ...
    \begin{equation}\label{eq:p2s2-basic}
        \symcal{F}[\e^{-a|x|}] = \frac{2a}{a^2+\omega^2}.
    \end{equation}
    利用欧拉公式 $\cos bx = \frac{1}{2}\left( \e^{\ii bx} + \e^{-\ii bx} \right)$ 和频移性质，可得
    \begin{equation}\label{eq:p2s2-shift}
        \begin{aligned}
            F(\omega) & = \frac{1}{2} \left[ \symcal{F}[\e^{-a|x|}]_{\omega \to \omega - b} + \symcal{F}[\e^{-a|x|}]_{\omega \to \omega + b} \right] \\
...
\begin{solution}
    ...
    奇延拓后的函数为 $f_o(x) = \begin{dcases} \e^{-x}, & x > 0, \\ 0, & x = 0, \\ -\e^x, & x < 0, \end{dcases}$ 图形如下：
    \begin{center}
        \begin{tikzpicture}[scale=0.8]
            \draw[->] (-4,0) -- (4,0) node[right] {$x$};
            \draw[->] (0,-1.5) -- (0,1.5) node[above] {$y$};
...
\begin{problem}{Fourier变换与狄利克雷积分}{Fourier-Transform-Dirichlet-Integral}
    求函数 $f(x) = \begin{dcases} 0, & |x| > 1, \\ 1, & |x| < 1 \end{dcases}$ 的 Fourier 变换．由此证明
    \begin{equation}
        \int_{0}^{+\infty} \frac{\sin \alpha \cos \alpha x}{\alpha} \d\alpha = \begin{dcases} \frac{\uppi}{2}, & |x| < 1, \\ \frac{\uppi}{4}, & |x| = 1, \\ 0, & |x| > 1. \end{dcases}
    \end{equation}
\end{problem}
...
\begin{solution}
    ...
    \begin{equation}
        \boxed{\int_{0}^{+\infty} \frac{\sin \alpha \cos \alpha x}{\alpha} \d\alpha = \begin{dcases} \frac{\uppi}{2}, & |x| < 1, \\ \frac{\uppi}{4}, & |x| = 1, \\ 0, & |x| > 1. \end{dcases}}
    \end{equation}
\end{solution}
