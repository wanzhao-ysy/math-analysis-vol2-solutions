...
\begin{problem}{反常积分的收敛性}{Improper-Integrals-Convergence-2}
    研究下列积分的条件收敛性与绝对收敛性：
    \begin{enumerate}
...
\begin{solution}
    ...
    第三，当 $\alpha \leqslant 0$ 即 $p \geqslant 2$ 时．由于被积函数 $t^{-\alpha} \sin t$ 在 $t \to +\infty$ 时不趋于 $0$，对于任意 $n \in \symbb{N}^*$，考虑区间 $[2n\uppi + \frac{\uppi}{4}, 2n\uppi + \frac{3\uppi}{4}]$，有：
    \begin{equation}
...
\begin{myproof}
    ...
    因为 $\int_0^{+\infty} f(t)g(t) \d t$ 收敛，对于任意 $\varepsilon > 0$，存在充分大的 $x_0 > 0$，使得
    \begin{equation}
        \int_{x_0}^{+\infty} f(t) g(t) \d t < \frac{\varepsilon}{2}.
    \end{equation}

    对于此固定的 $x_0$，由于 $\lim_{x\to +\infty} g(x) = 0$，存在 $X > x_0$，使得当 $x > X$ 时，有
    \begin{equation}
        g(x) \int_0^{x_0} f(t) \d t < \frac{\varepsilon}{2}.
    \end{equation}

    综上所述，当 $x > X$ 时，有
    \begin{equation}
        0 \leqslant g(x) \int_0^x f(t) \d t < \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon.
    \end{equation}
