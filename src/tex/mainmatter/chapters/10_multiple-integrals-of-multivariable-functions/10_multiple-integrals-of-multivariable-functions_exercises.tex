...
\begin{problem}{二重积分}{Double-Integral}
    设 $D = \{(x, y) \mid x^2 + y^2 \leqslant 1\}$．求 $I = \iint_D \left| \frac{x+y}{\sqrt{2}} - x^2 - y^2 \right| \d x \d y$．
\end{problem}
...
\begin{solution}
    ...
    当 $\sin \alpha \leqslant 0$ 时，即 $\alpha \in [\uppi, 2\uppi]$，有
    \begin{equation}
...
\begin{problem}{二重积分的应用与极坐标面积}{Lemniscate-Circle-Intersection}
    试求圆盘 $(x-a)^2 + (y-a)^2 \leqslant a^2$ 与曲线 $(x^2 + y^2)^2 = 8a^2 xy$ 所围部分相交的区域 $D$ 的面积．
\end{problem}
...
\begin{solution}
    ...
    \end{equation}
    根据题意，区域 $D$ 是圆盘内部与双纽线内部的交集．由于双纽线在第一象限的部分满足 $r \leqslant 2a\sqrt{\sin 2\theta}$，而圆盘内部满足 $a(\cos \theta + \sin \theta) - a \sqrt{\sin 2\theta} \leqslant r \leqslant a(\cos \theta + \sin \theta) + a \sqrt{\sin 2\theta}$．令两曲线边界相交，有
    \begin{equation}
...
\begin{myproof}
    ...
    \end{equation}
    对式 \zcref{eq:F-t-form} 关于 $t$ 求导，根据含参变量积分求导法则：
    \begin{equation}
        \begin{aligned}
    ...
    \ansitem{2}
    考虑式 \zcref{eq:F-t-form} 中的第二项积分 $I = \iint_{[0,t]^2} xy f'(xy) \, \d x \d y$．利用累次积分：
    \begin{equation}
...
\begin{problem}{Poincaré 不等式}{Poincare-Inequality}
    设 $\varphi(x), \psi(x)$ 是 $[a, b]$ 上的连续函数，$f(x, y)$ 在区域 $D = \{(x, y) : a \leqslant x \leqslant b, \varphi(x) \leqslant y \leqslant \psi(x)\}$ 上连续可微，且有 $f(x, \varphi(x)) = 0$，则存在 $M > 0$，使得
    \begin{equation}
        \iint_D f^2(x, y) \d x \d y \leqslant M \iint_D (f'_y(x, y))^2 \d x \d y.
    \end{equation}
\end{problem}
...
\begin{myproof}
    ...
    \begin{equation}
        \begin{aligned}
            f^2(x, y) & = \left( \int_{\varphi(x)}^y 1 \cdot f'_t(x, t) \d t \right)^2                                               \\
                      & \leqslant \left( \int_{\varphi(x)}^y 1^2 \d t \right) \left( \int_{\varphi(x)}^y (f'_t(x, t))^2 \d t \right) \\
                      & \leqslant (y - \varphi(x)) \int_{\varphi(x)}^{\psi(x)} (f'_y(x, y))^2 \d y.
    ...
    \begin{equation}
        \begin{aligned}
            \int_{\varphi(x)}^{\psi(x)} f^2(x, y) \d y & \leqslant \int_{\varphi(x)}^{\psi(x)} (y - \varphi(x)) \d y \cdot \int_{\varphi(x)}^{\psi(x)} (f'_y(x, y))^2 \d y \\
    ...
    \begin{equation}
        \begin{aligned}
            \iint_D f^2(x, y) \d x \d y & = \int_a^b \left( \int_{\varphi(x)}^{\psi(x)} f^2(x, y) \d y \right) \d x                \\
                                        & \leqslant \int_a^b M \left( \int_{\varphi(x)}^{\psi(x)} (f'_y(x, y))^2 \d y \right) \d x \\
    ...
    综上所述，存在常数 $M$，使得
    \begin{equation}
        \boxed{\iint_D f^2(x, y) \d x \d y \leqslant M \iint_D (f'_y(x, y))^2 \d x \d y.}
    \end{equation}
\end{myproof}

\begin{problem}{多重积分}{Multiple-Integral}
    设 $a > 0, \Omega_n(a) : x_1 + \cdots + x_n \leqslant a, x_i \geqslant 0 (i = 1, 2, \cdots , n)$．求积分
    \begin{equation}
...
\begin{solution}
    通过对变量 $x_n$ 采用累次积分法，积分区域 $\Omega_n(a)$ 可表示为 $0 \leqslant x_n \leqslant a$ 且其投影区域为 $\Omega_{n-1}(a - x_n)$．由此建立如下递推关系
    \begin{equation}
        \begin{aligned}
